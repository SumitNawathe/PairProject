...
//			setTris(Mesh.loadFromObjFile("Models/Ship Model 4.obj", "Textures/Ship Model 4 Map.png").getTris());
	...
	public void update(GraphicsPanel graphicsPanel) {
		...
		if (fireMod % 5 == 0) {
			...
			graphicsPanel.fireBullet(getPos().clone().plus(new Vector(0, 5*3.5*Math.cos(firePos*Math.PI/2+theta+Math.PI/4), 5*3.5*Math.sin(firePos*Math.PI/2+theta+Math.PI/4))), new Vector(-1, 0, 0), 1.5);
		}
		fireMod++;
	}
